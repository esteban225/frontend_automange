...
@Component({
  ...
  templateUrl: './register-user.component.html', // Especifica el archivo HTML asociado al componente
  styleUrls: ['./register-user.component.scss'] // Especifica el archivo de estilos CSS del componente
})
export class RegisterUserComponent implements OnInit { // Declara la clase del componente y la implementa con OnInit
  ...
  checkPasswordStrength() {
    const password = this.registerForm.get('password')?.value; // Obtiene el valor del campo de contraseña
    const strongRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/; // Expresión regular para validar contraseña fuerte

    this.passwordStrength = strongRegex.test(password) ? 'strong' : 'weak'; // Si cumple con la expresión, se considera "fuerte"
  }
